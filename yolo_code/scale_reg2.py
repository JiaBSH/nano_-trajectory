...
if torch.cuda.is_available():
    ...
    print(f"Torch:GPU is available, using {torch.cuda.get_device_name(device)}")
else:
    device = torch.device("cpu")  # 使用 CPU
    print("Torch:GPU not available, using CPU")
import paddle
if paddle.is_compiled_with_cuda():
    print("Paddle:GPU is available!")
else:
    print("Paddle:GPU is not available.")

# ========== 1️⃣ 加载模型 ==========
yolo_model = YOLO(r'E:\cq_instance_post\yolo_code\runs\detect\train5\weights\best.pt')  # YOLO模型路径
ocr = PaddleOCR(use_textline_orientation=True, lang='en')  # OCR模型（可识别 μm）
...
def recognize_scale_text(image, text_box):
    ...
    if isinstance(result[0], dict) and 'rec_texts' in result[0]:
        ...
        print(f"OCR识别结果: {list(zip(rec_texts, rec_scores))}")
        # 按置信度排序取最高的
        best_idx = int(np.argmax(rec_scores))
        text = rec_texts[best_idx].strip()
    else:
        # 兼容旧版 PaddleOCR
        try:
...
def parse_scale_text(text):
    match = re.search(r"([\d\.]+)\s*([munμ]+)", text, re.IGNORECASE)
    if not match:
        print(f"无法解析文字为数值单位: {text}")
        return None, None
    value = float(match.group(1))
    unit = match.group(2).replace("u", "μ").replace("µ", "μ")
    return value, unit
...
def process_image(image_path):
    ...
    if scale_text_box is not None:
        ...
        cv2.putText(image, text, (scale_text_box[0], scale_text_box[1]-5),
...
if __name__ == "__main__":
    process_image('D:\\code\\cq_instance_post\\big_data\\yolo_dataset\\images\\val\\20250107-2.jpg')
